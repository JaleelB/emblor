...
import type { EmblorRootProps, TagValue } from '@emblor/types';
import {
...
export const EmblorRoot = React.forwardRef<HTMLElement, EmblorRootProps>(function EmblorRoot(props, forwardedRef) {
  ...
  React.useImperativeHandle(forwardedRef, function resolveImperativeHandle() {
    return rootRef.current as HTMLElement | null;
  });
  ...
  const commitTag = React.useCallback(
    function handleCommit(rawValue: string, meta?: { source: 'keyboard' | 'paste' | 'blur' }): boolean {
      ...
      let committedValue = '';
      setTagValues(function apply(previous) {
        const next = [...previous, validation.value];
        committedIndex = next.length - 1;
  ...
  const removeTag = React.useCallback(
    function handleRemove(indexToRemove: number) {
      ...
      setTagValues(function apply(previous) {
        const next = previous.filter(function filterValue(_, index) {
          return index !== indexToRemove;
        });
        return next;
      });
      const nextLength = tagValues.length - 1;
